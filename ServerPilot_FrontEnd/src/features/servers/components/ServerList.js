    ...
    <RootContainer>
      <GlassCard>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h5" component="h2" sx={{ color: '#fff' }}>Servers</Typography>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
        ...
        {servers.length === 0 && !error && (
          <Typography sx={{mt: 2, textAlign: 'center', color: 'rgba(255, 255, 255, 0.7)'}}>No servers found for this customer.</Typography>
        )}
        <Menu
            ...
            <TableBody>
              {servers.map((server) => (
                <React.Fragment key={server.id}>
                  <TableRow hover sx={{ '&:last-child td, &:last-child th': { border: 0 }, '& .MuiTableCell-root': { color: '#fff', borderBottom: '1px solid rgba(255, 255, 255, 0.1)' } }}>
                    <TableCell component="th" scope="row">
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <DnsIcon sx={{ mr: 1.5, color: 'rgba(255, 255, 255, 0.7)' }} />
                    ...
                    <TableCell>
                      <Chip
                        ...
                        variant="outlined"
                        sx={{
                            borderColor: server.is_active ? 'rgba(102, 187, 106, 0.7)' : 'rgba(244, 67, 54, 0.7)',
                            color: server.is_active ? '#66bb6a' : '#f44336',
                        }}
                      />
                    </TableCell>
                    <TableCell align="center">
                      <Tooltip title={
                        ...
                        <IconButton onClick={() => handleTestConnection(server.id)} disabled={testConnectionStatus[server.id]?.testing} sx={{ color: 'rgba(255, 255, 255, 0.8)' }}>
                          {testConnectionStatus[server.id]?.testing ? <CircularProgress size={24} sx={{ color: '#FE6B8B' }} /> :
                    ...
                    <TableRow>
                      <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={5}>
                        <Collapse in={true} timeout="auto" unmountOnExit>
                          <Box sx={{ margin: 1, p: 2, border: '1px dashed', borderColor: 'rgba(255, 255, 255, 0.3)', borderRadius: 1, bgcolor: testConnectionStatus[server.id]?.status === 'error' ? 'rgba(255, 0, 0, 0.1)' : 'rgba(0, 0, 0, 0.1)' }}>
        ...
        <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog} PaperProps={{ sx: { background: 'rgba(30, 40, 57, 0.9)', color: '#fff', backdropFilter: 'blur(5px)' } }}>
          ...
          <DialogActions>
            <Button onClick={handleCloseDeleteDialog} sx={{ color: '#fff' }}>Cancel</Button>
            <Button onClick={handleDelete} color="error">Delete</Button>
          </DialogActions>
        </Dialog>
        <Dialog open={changePasswordDialogOpen} onClose={handleCloseChangePasswordDialog} PaperProps={{ sx: { background: 'rgba(30, 40, 57, 0.9)', color: '#fff', backdropFilter: 'blur(5px)' } }}>
          ...
          <DialogContent>
            ...
            />
            <Button onClick={generateComplexPassword} sx={{ mt: 2, color: '#FE6B8B' }}>
              Generate Complex Password
            </Button>
          </DialogContent>
          <DialogActions>
            <Button onClick={handleCloseChangePasswordDialog} sx={{ color: '#fff' }}>Cancel</Button>
            <Button onClick={handleChangePassword} sx={{ color: '#FE6B8B' }}>Change Password</Button>
          </DialogActions>
        </Dialog>
        {/* Server Info Modal */}
        {infoModalOpen && (
          <Dialog open={infoModalOpen} onClose={handleCloseInfoModal} fullWidth maxWidth="md" PaperProps={{ sx: { background: 'rgba(30, 40, 57, 0.9)', color: '#fff', backdropFilter: 'blur(5px)' } }}>
            <DialogTitle>Server Statistics: {serverInfo?.serverName}</DialogTitle>
            <DialogContent>
              {infoLoading ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', my: 4 }}><CircularProgress sx={{ color: '#FE6B8B' }} /></Box>
              ) : serverInfo?.error ? (
                <Alert severity="error" sx={{ background: 'rgba(211, 47, 47, 0.8)', color: '#fff' }}>{serverInfo.error}</Alert>
              ) : serverInfo?.data ? (
                (() => {
            ...
            <DialogActions>
              <Button onClick={handleCloseInfoModal} sx={{ color: '#fff' }}>Close</Button>
            </DialogActions>
          </Dialog>
        )}
        {/* Add/Edit Server Modal */}
        <Dialog open={isModalOpen} onClose={handleCloseModal} maxWidth="md" fullWidth PaperProps={{ sx: { background: 'rgba(30, 40, 57, 0.9)', color: '#fff', backdropFilter: 'blur(5px)' } }}>
